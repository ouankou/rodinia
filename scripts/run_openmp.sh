...
if [[ ! -f "${DATA_DIR}/leukocyte/testfile.avi" ]]; then
  "${ROOT_DIR}/scripts/generate_data.sh" --only leukocyte
fi

if [[ ${RUN_MUMMERGPU} -eq 1 && ! -f "${DATA_DIR}/mummergpu/NC_003997_q100bp.fna" ]]; then
  "${ROOT_DIR}/scripts/generate_data.sh" --only mummergpu
fi
