...
int main(int argc, char *argv[]) {
  ...
  if (data_dir != NULL && data_dir[0] != '\0') {
    snprintf(input_path, sizeof(input_path), "%s/srad/image.pgm", data_dir);
  } else {
    snprintf(input_path, sizeof(input_path), "../../../data/srad/image.pgm");
  }
  read_graphics(input_path, image_ori, image_ori_rows, image_ori_cols, 1);
...
#pragma omp target teams distribute parallel for shared(                       \
  ...
  const char *output_dir = getenv("RODINIA_BENCH_OUTPUT_DIR");
  char output_path[4096];
  if (output_dir != NULL && output_dir[0] != '\0') {
    snprintf(output_path, sizeof(output_path), "%s/image_out.pgm", output_dir);
  } else {
    snprintf(output_path, sizeof(output_path), "image_out.pgm");
  }
  write_graphics(output_path, image, Nr, Nc, 1, 255);
