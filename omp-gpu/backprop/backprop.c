...
void bpnn_randomize_weights(float *w, int m, int n) {
  ...
  for (i = 0; i <= m; i++) {
    for (j = 0; j <= n; j++) {
      w[i * stride + j] = (float)((double)rand() / (double)RAND_MAX);
    }
  }
}
...
void bpnn_adjust_weights(float *delta, int ndelta, float *ly, int nly, float *w,
                         float *oldw) {
  int stride = ndelta + 1;
  int weight_count = (nly + 1) * (ndelta + 1);
  ly[0] = 1.0;
#pragma omp target update to(ly[0:1])

#pragma omp target teams distribute parallel for                                  \
    map(to : delta [0:ndelta + 1], ly [0:nly + 1])                                \
    map(tofrom : w [0:weight_count], oldw [0:weight_count]) collapse(2)
  for (int j = 1; j <= ndelta; j++) {
    for (int k = 0; k <= nly; k++) {
      float new_dw =
          ((ETA * delta[j] * ly[k]) + (MOMENTUM * oldw[k * stride + j]));
      w[k * stride + j] += new_dw;
...
void bpnn_save(BPNN *net, char *filename) {
  ...
  */
  ...
  int hidden_stride = n3 + 1;
  mem = (char *)malloc((unsigned)((n2 + 1) * (n3 + 1) * sizeof(float)));
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      dvalue = w[i * hidden_stride + j];
      fastcopy(&mem[memcnt], &dvalue, sizeof(float));
      memcnt += sizeof(float);
    }
  }
...
BPNN *bpnn_read(char *filename) {
  ...
  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      fastcopy(&(new->input_weights[i * input_stride + j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      fastcopy(&(new->hidden_weights[i * hidden_stride + j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
