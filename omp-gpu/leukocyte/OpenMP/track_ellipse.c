...
void ellipsetrack(avi_t *video, double *xc0, double *yc0, int Nc, int R, int Np, int Nf) {
	...
	*/
	...
	if (t == NULL) {
		fprintf(stderr, "Error: Memory allocation failed for t\n");
		exit(EXIT_FAILURE);
	}
	...
	double **ri_all = alloc_2d_double(Nc, Np);
	if (xc == NULL || yc == NULL || r == NULL || x == NULL || y == NULL || ri_all == NULL) {
		fprintf(stderr, "Error: Memory allocation failed for tracking buffers\n");
		free(t);
		free_2d_double(xc);
		free_2d_double(yc);
		free_3d_double(r);
...
MAT *MGVF(MAT *I, double vx, double vy) {
	...
	*/
	...
	int *rowU = (int *) checked_calloc_array((size_t)m, sizeof(int));
	int *rowD = (int *) checked_calloc_array((size_t)m, sizeof(int));
	int *colL = (int *) checked_calloc_array((size_t)n, sizeof(int));
	int *colR = (int *) checked_calloc_array((size_t)n, sizeof(int));
	if (rowU == NULL || rowD == NULL || colL == NULL || colR == NULL) {
		fprintf(stderr, "Error: Memory allocation failed for MGVF index buffers\n");
		free(rowU);
		free(rowD);
		free(colL);
		free(colR);
...
void ellipseevolve(MAT *f, double *xc0, double *yc0, double *r0, double *t, int Np, double Er, double Ey) {
	...
	*/
	...
	if (r == NULL) {
		fprintf(stderr, "Error: Memory allocation failed for ellipse radius buffer\n");
		exit(EXIT_FAILURE);
	}
	...
	VEC *x = v_get(Np);
	VEC *y = v_get(Np);
	if (r_old == NULL || x == NULL || y == NULL) {
		fprintf(stderr, "Error: Memory allocation failed for ellipse evolution buffers\n");
		free(r);
		free(r_old);
		if (x != NULL) v_free(x);
		if (y != NULL) v_free(y);
...
double **alloc_2d_double(int x, int y) {
	...
	if (checked_mul_size((size_t)x, (size_t)y, &element_count) != 0) {
		return NULL;
	}
	double *data = (double *) calloc(element_count, sizeof(double));
	double **pointers = (double **) malloc(sizeof(double *) * (size_t)x);
	if (data == NULL || pointers == NULL) {
		free(data);
		free(pointers);
		return NULL;
	}
	...
	int i;
	for (i = 0; i < x; i++) {
		pointers[i] = data + ((size_t)i * (size_t)y);
	}
...
double ***alloc_3d_double(int x, int y, int z) {
	...
	size_t xy_count;
	size_t element_count;
	if (checked_mul_size((size_t)x, (size_t)y, &xy_count) != 0 ||
	    checked_mul_size(xy_count, (size_t)z, &element_count) != 0) {
		return NULL;
	}
	double *data = (double *) calloc(element_count, sizeof(double));
	double **pointers_to_data = (double **) malloc(sizeof(double *) * xy_count);
	double ***pointers_to_pointers = (double ***) malloc(sizeof(double **) * (size_t)x);
	if (data == NULL || pointers_to_data == NULL || pointers_to_pointers == NULL) {
		free(data);
		free(pointers_to_data);
		free(pointers_to_pointers);
		return NULL;
	}

	// Make the pointers point to the data
	size_t data_index;
	for (data_index = 0; data_index < xy_count; data_index++) {
		pointers_to_data[data_index] = data + (data_index * (size_t)z);
	}
	int i;
	for (i = 0; i < x; i++) {
		pointers_to_pointers[i] = pointers_to_data + ((size_t)i * (size_t)y);
	}
