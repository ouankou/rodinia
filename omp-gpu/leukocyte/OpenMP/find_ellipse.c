...
static void allocate_index_buffers(int n, int **aindex, int **bindex, int **cindex, int **dindex, const char *context) {
	if (n < 1) {
		fprintf(stderr, "Error: Invalid index count in %s\n", context);
		exit(EXIT_FAILURE);
	}
	...
	*aindex = checked_malloc_array((size_t)n, sizeof(int));
	*bindex = checked_malloc_array((size_t)n, sizeof(int));
	*cindex = checked_malloc_array((size_t)n, sizeof(int));
	*dindex = checked_malloc_array((size_t)n, sizeof(int));
	if (*aindex == NULL || *bindex == NULL || *cindex == NULL || *dindex == NULL) {
...
MAT * ellipsematching(MAT * grad_x, MAT * grad_y) {
  ...
  */
  ...
  size_t grad_x_size;
  size_t grad_y_size;
  if (checked_mul_size((size_t)height, (size_t)width, &gicov_size) != 0 ||
      checked_mul_size((size_t)grad_x_m, (size_t)grad_x_n, &grad_x_size) != 0 ||
      checked_mul_size((size_t)grad_y_m, (size_t)grad_y_n, &grad_y_size) != 0) {
    fprintf(stderr, "Error: allocation size overflow in ellipsematching\n");
    m_free(gicov);
    exit(EXIT_FAILURE);
  }

  double *gicov_dev  = (double *)checked_malloc_array(gicov_size, sizeof(double));
  double *grad_x_dev = (double *)checked_malloc_array(grad_x_size, sizeof(double));
  double *grad_y_dev = (double *)checked_malloc_array(grad_y_size, sizeof(double));
  if (gicov_dev == NULL || grad_x_dev == NULL || grad_y_dev == NULL) {
    fprintf(stderr, "Error: Memory allocation failed in ellipsematching\n");
    free(gicov_dev);
    free(grad_x_dev);
    free(grad_y_dev);
    m_free(gicov);
  ...
  memset(gicov_dev, 0, gicov_size * sizeof(double));
...
#pragma omp target teams distribute parallel for collapse(2) private(i,j,k,n,x,y) \
    map(to:    sin_angle[0:NPOINTS], cos_angle[0:NPOINTS], tX[0:NCIRCLES*NPOINTS], tY[0:NCIRCLES*NPOINTS], grad_x_dev[0:grad_x_size], grad_y_dev[0:grad_y_size]) \
    map(tofrom:  gicov_dev[0:gicov_size])
	for (i = MaxR; i < width - MaxR; i++) {
        ...
        for (n = 0; n < NPOINTS; n++) {
          ...
          double val = grad_x_dev[y*grad_x_n+x]*cos_angle[n] + grad_y_dev[y*grad_y_n+x]*sin_angle[n];
          sum += val;
        }
...
MAT * dilate_f(MAT * img_in, MAT * strel) {
  ...
  size_t img_size;
  size_t strel_size;
  if (checked_mul_size((size_t)img_m, (size_t)img_n, &img_size) != 0 ||
      checked_mul_size((size_t)strel_m, (size_t)strel_n, &strel_size) != 0) {
    fprintf(stderr, "Error: allocation size overflow in dilate_f\n");
    m_free(dilated);
    exit(EXIT_FAILURE);
  }

  double *dilated_dev = (double *)checked_malloc_array(img_size, sizeof(double));
  double *img_dev     = (double *)checked_malloc_array(img_size, sizeof(double));
  double *strel_dev   = (double *)checked_malloc_array(strel_size, sizeof(double));
  if (dilated_dev == NULL || img_dev == NULL || strel_dev == NULL) {
    fprintf(stderr, "Error: Memory allocation failed in dilate_f\n");
    free(dilated_dev);
    free(img_dev);
    free(strel_dev);
    m_free(dilated);
...
#pragma omp target teams distribute parallel for \
    map(to: img_dev[0:img_size], strel_dev[0:strel_size]) \
    map(from: dilated_dev[0:img_size]) collapse(2)
...
MAT * TMatrix(unsigned int N, unsigned int M)
{
	...
	aindex = checked_malloc_array(N, sizeof(int));
	bindex = checked_malloc_array(N, sizeof(int));
	cindex = checked_malloc_array(N, sizeof(int));
	dindex = checked_malloc_array(N, sizeof(int));
	if (aindex == NULL || bindex == NULL || cindex == NULL || dindex == NULL) {
		fprintf(stderr, "Error: Memory allocation failed in TMatrix\n");
		free(dindex);
		free(cindex);
		free(bindex);
		free(aindex);
...
VEC * getsampling(MAT * m, int ns)
{
	...
	allocate_index_buffers(N, &aindex, &bindex, &cindex, &dindex, "getsampling");
...
VEC * getfdriv(MAT * m, int ns)
{
	...
	allocate_index_buffers(N, &aindex, &bindex, &cindex, &dindex, "getfdriv");
...
void splineenergyform01(MAT * Cx, MAT * Cy, MAT * Ix, MAT * Iy, int ns, double delta, double dt, int typeofcell)
{
	...
	N = Cx->m;
	allocate_index_buffers(N, &aindex, &bindex, &cindex, &dindex, "splineenergyform01");
