...
float* block;
static float* coord_d;

static void free_coord_buffer() {
  free(coord_d);
  coord_d = NULL;
}
...
static void* checked_calloc_array(size_t count, size_t elem_size) {
  size_t bytes;
  if (!checked_mul_size(count, elem_size, &bytes)) {
    return NULL;
  }
...
static void free_cluster_state() {
...
static bool allocate_cluster_state(long num) {
  if (num < 0) {
    fprintf(stderr,"invalid cluster state size!\n");
    return false;
  }

  switch_membership = (bool*)checked_malloc_array((size_t)num, sizeof(bool));
  is_center = (bool*)checked_calloc_array((size_t)num, sizeof(bool));
  center_table = (int*)checked_malloc_array((size_t)num, sizeof(int));
  if( num != 0 && (switch_membership == NULL || is_center == NULL || center_table == NULL) ) {
    fprintf(stderr,"not enough memory for cluster state!\n");
    free_cluster_state();
    return false;
  }
  return true;
...
static float dist_flat(long p1, long p2, int dim, const float *coord)
{
  float result = 0.0f;
  for (int i = 0; i < dim; i++) {
    float diff = coord[p1 * dim + i] - coord[p2 * dim + i];
    result += diff * diff;
  }
  return result;
}
...
static void copy_flat_coordinates(Points *points)
{
  long num = points->num;
  int dim = points->dim;
  for (long ii = 0; ii < num; ii++) {
    for (int jj = 0; jj < dim; jj++) {
      coord_d[ii * dim + jj] = points->p[ii].coord[jj];
    }
  }
}
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid==0 )    {
    size_t work_count;
    if (!checked_mul_size((size_t)stride, (size_t)(nproc + 1), &work_count)) {
      fprintf(stderr,"not enough memory for work buffers!\n");
      exit(1);
    }
    work_mem = (double*)checked_malloc_array(work_count, sizeof(double));
    if( work_mem == NULL ) {
      fprintf(stderr,"not enough memory for work buffers!\n");
      exit(1);
    }
  ...
  */
  ...
  long local_count = k2 - k1;
  double *lower_contrib = NULL;
  int *assign_contrib = NULL;
  if (local_count > 0) {
    lower_contrib = (double *)calloc(local_count, sizeof(double));
    assign_contrib = (int *)calloc(local_count, sizeof(int));
    if (lower_contrib == NULL || assign_contrib == NULL) {
      fprintf(stderr, "not enough memory for pgain contributions\n");
      free(lower_contrib);
      free(assign_contrib);
      exit(1);
    }
  ...
  if (local_count > 0) {
    #pragma omp target update to(switch_membership [k1:local_count])
    #pragma omp target teams distribute parallel for                                \
        map(alloc : d_points [0:num], coord_d [0:num * dim],                        \
                    switch_membership [0:num])                                      \
        map(from : lower_contrib [0:local_count], assign_contrib [0:local_count])   \
        reduction(+ : cost_of_opening_x)
    for ( i = k1; i < k2; i++ ) {
      float x_cost = dist_flat(i, x, dim, coord_d) * d_points[i].weight;
      float current_cost = d_points[i].cost;
      int assign = d_points[i].assign;
      assign_contrib[i - k1] = assign;
      ...
      if ( x_cost < current_cost ) {
        ...
        lower_contrib[i - k1] = current_cost - x_cost;
      }
    }
    #pragma omp target update from(switch_membership [k1:local_count])

    for ( i = k1; i < k2; i++ ) {
      if (!switch_membership[i]) {
        lower[center_table[assign_contrib[i - k1]]] += lower_contrib[i - k1];
      }
    }
  }
  free(assign_contrib);
  ...
  if ( gl_cost_of_opening_x < 0 ) {
    ...
    int local_count = k2 - k1;
    #pragma omp target teams distribute parallel for                                \
        map(alloc : d_points [0:num], coord_d [0:num * dim],                        \
                    switch_membership [0:num])                                      \
        map(to : center_table [0:num], gl_lower [0:stride])
    for ( int i = k1; i < k2; i++ ) {
      bool close_center = gl_lower[center_table[d_points[i].assign]] > 0 ;
      if ( switch_membership[i] || close_center ) {
					// Either i's median (which may be i itself) is closing,
					// or i is closer to x than to its current median
					d_points[i].cost = d_points[i].weight *
						dist_flat(i, x, dim, coord_d);
					d_points[i].assign = x;
      }
    }
	  ...
	  int pid, pthread_barrier_t* barrier)
{
  ...
  copy_flat_coordinates(points);
  Point *d_points = points->p;
  #pragma omp target data map(to : coord_d [0:numberOfPoints * points->dim])        \
                          map(tofrom : d_points [0:numberOfPoints],                \
                                        switch_membership [0:numberOfPoints])
  {
...
void outcenterIDs( Points* centers, long* centerIDs, char* outfile ) {
  ...
  int* is_a_median = NULL;
  if( centers->num > 0 ) {
    is_a_median = (int*)calloc( centers->num, sizeof(int) );
    if( is_a_median == NULL ) {
      fprintf(stderr, "not enough memory for output median table\n");
      fclose(fp);
      exit(1);
    }
  }
		    ...
		    long chunksize, long centersize, char* outfile )
{
  size_t chunk_coords;
  size_t coord_coords;
  size_t center_coords;
  if (!checked_mul_size((size_t)chunksize, (size_t)dim, &chunk_coords) ||
      !checked_mul_size((size_t)(chunksize > centersize ? chunksize : centersize), (size_t)dim, &coord_coords) ||
      !checked_mul_size((size_t)centersize, (size_t)dim, &center_coords)) {
    fprintf(stderr,"not enough memory for a chunk!\n");
    exit(1);
  }

  block = (float*)checked_malloc_array( chunk_coords, sizeof(float) );
  coord_d = (float*)checked_malloc_array(coord_coords, sizeof(float));
  float* centerBlock = (float*)checked_malloc_array(center_coords, sizeof(float) );
  long* centerIDs = (long*)checked_malloc_array(center_coords, sizeof(long));

  if( block == NULL || coord_d == NULL || centerBlock == NULL || centerIDs == NULL ) {
    fprintf(stderr,"not enough memory for a chunk!\n");
    free(block);
    free(centerBlock);
    free(centerIDs);
  ...
  points.p = (Point *)checked_malloc_array((size_t)chunksize, sizeof(Point));
  if( points.p == NULL ) {
    fprintf(stderr,"not enough memory for a chunk!\n");
    free(block);
    free(centerBlock);
  ...
  Points centers;
  centers.dim = dim;
  centers.p = (Point *)checked_malloc_array((size_t)centersize, sizeof(Point));
  if( centers.p == NULL ) {
    fprintf(stderr,"not enough memory for a chunk!\n");
    free(block);
    free(centerBlock);
  ...
  while(1) {
    ...
    if( !allocate_cluster_state(points.num) ) {
      free(block);
      free(centerBlock);
      free(centerIDs);
      free(points.p);
  ...
  if( !allocate_cluster_state(centers.num) ) {
    free(block);
    free(centerBlock);
    free(centerIDs);
    free(points.p);
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  if (argc < 9 || argc > 10) {
    ...
    fprintf(stderr,"  nproc:       Accepted for OpenMP runner compatibility; GPU offload execution uses target teams\n");
    fprintf(stderr,"\n");
  ...
  int in_written = snprintf(infilename, sizeof(infilename), "%s", argv[7]);
  int out_written = snprintf(outfilename, sizeof(outfilename), "%s", argv[8]);
  if (in_written < 0 || (size_t)in_written >= sizeof(infilename) ||
      out_written < 0 || (size_t)out_written >= sizeof(outfilename)) {
    fprintf(stderr, "input or output filename is too long\n");
    exit(1);
  }
  nproc = 1;
