...
fp cam(fp timeinst,
	...
	// Ca/CaM parameters
	if (Mg <= 1){
		Kd02 = 0.0025f*(1+K/0.94f-Mg/0.012f)*(1+K/8.1f+Mg/0.022f);							// [uM^2]
		Kd24 = 0.128f*(1+K/0.64f+Mg/0.0014f)*(1+K/13.0f-Mg/0.153f);							// [uM^2]
	}
	else{
		Kd02 = 0.0025f*(1+K/0.94f-1/0.012f+(Mg-1)/0.060f)*(1+K/8.1f+1/0.022f+(Mg-1)/0.068f);   // [uM^2]
		Kd24 = 0.128f*(1+K/0.64f+1/0.0014f+(Mg-1)/0.005f)*(1+K/13.0f-1/0.153f+(Mg-1)/0.150f);  // [uM^2]
	}
	k20 = 10;																			// [s^-1]
	...
	// CaM buffering (B) parameters
	k0Boff = 0.0014f;																	// [s^-1]
	k0Bon = k0Boff/0.2f;																	// [uM^-1 s^-1] kon = koff/Kd
	...
	// Wi Wa Wt Wp
	kbi = 2.2f;																			// [s^-1] (Ca4CaM dissocation from Wb)
	kib = kbi/33.5e-3f;																	// [uM^-1 s^-1]
	kpp1 = 1.72f;																		// [s^-1] (PP1-dep dephosphorylation rates)
	Kmpp1 = 11.5f;																		// [uM]
	kib2 = kib;
	kb2i = kib2*5;
	kb24 = k24;
	...
	// CaN parameters
	kcanCaoff = 1;																		// [s^-1]
	kcanCaon = kcanCaoff/0.5f;															// [uM^-1 s^-1]
	kcanCaM4on = 46;																	// [uM^-1 s^-1]
	kcanCaM4off = 0.0013f;																// [s^-1]
	...
	// CaM Reaction fluxes
	rcn02 = k02*powf(Ca,2)*CaM - k20*Ca2CaM;
	rcn24 = k24*powf(Ca,2)*Ca2CaM - k42*Ca4CaM;

	// CaM buffer fluxes
	B = Btot - CaMB - Ca2CaMB - Ca4CaMB;
	rcn02B = k02B*powf(Ca,2)*CaMB - k20B*Ca2CaMB;
	rcn24B = k24B*powf(Ca,2)*Ca2CaMB - k42B*Ca4CaMB;
	...
	rcnCa4CaN = kcanCaon*powf(Ca,2)*Ca2CaN - kcanCaoff*Ca4CaN;
	rcn02CaN = k02can*powf(Ca,2)*CaMCa4CaN - k20can*Ca2CaMCa4CaN;
	rcn24CaN = k24can*powf(Ca,2)*Ca2CaMCa4CaN - k42can*Ca4CaMCa4CaN;
	...
	rcnCKb2b = kb24*powf(Ca,2)*Pb2 - kb42*Pb;
	rcnCKib = kib*Ca4CaM*Pix - kbi*Pb;
	T = Pb + Pt + Pt2 + Pa;
	kbt = 0.055f*T + 0.0074f*powf(T,2) + 0.015f*powf(T,3);
	rcnCKbt = kbt*Pb - kpp1*PP1tot*Pt/(Kmpp1+CaMKIItot*Pt);
	rcnCKtt2 = kt42*Pt - kt24*powf(Ca,2)*Pt2;
	...
	// CaM equations
	dCaM = 1e-3f*(-rcn02 - rcn0B - rcn0CaN);
	dCa2CaM = 1e-3f*(rcn02 - rcn24 - rcn2B - rcn2CaN + CaMKIItot*(-rcnCKib2 + rcnCKt2a) );
	dCa4CaM = 1e-3f*(rcn24 - rcn4B - rcn4CaN + CaMKIItot*(-rcnCKib+rcnCKta) );
	dCaMB = 1e-3f*(rcn0B-rcn02B);
	dCa2CaMB = 1e-3f*(rcn02B + rcn2B - rcn24B);
	dCa4CaMB = 1e-3f*(rcn24B + rcn4B);

	// CaMKII equations
	dPb2 = 1e-3f*(rcnCKib2 - rcnCKb2b + rcnCKt2b2);										// Pb2
	dPb = 1e-3f*(rcnCKib + rcnCKb2b - rcnCKbt);											// Pb
	dPt = 1e-3f*(rcnCKbt-rcnCKta-rcnCKtt2);												// Pt
	dPt2 = 1e-3f*(rcnCKtt2-rcnCKt2a-rcnCKt2b2);											// Pt2
	dPa = 1e-3f*(rcnCKta+rcnCKt2a-rcnCKai);												// Pa

	// CaN equations
	dCa4CaN = 1e-3f*(rcnCa4CaN - rcn0CaN - rcn2CaN - rcn4CaN);							// Ca4CaN
	dCaMCa4CaN = 1e-3f*(rcn0CaN - rcn02CaN);												// CaMCa4CaN
	dCa2CaMCa4CaN = 1e-3f*(rcn2CaN+rcn02CaN-rcn24CaN);									// Ca2CaMCa4CaN
	dCa4CaMCa4CaN = 1e-3f*(rcn4CaN+rcn24CaN);											// Ca4CaMCa4CaN
	...
	JCa = 1e-3f*(2*CaMKIItot*(rcnCKtt2-rcnCKb2b) - 2*(rcn02+rcn24+rcn02B+rcn24B+rcnCa4CaN+rcn02CaN+rcn24CaN)); // [uM/msec]
