...
__global__ void
mergeSortFirst(const float4 *input, float4 *result, int listsize) 
{
    // Block index
    int bx = blockIdx.x;
    // Thread index
    //int tx = threadIdx.x;
		if(bx*blockDim.x + threadIdx.x < listsize/4){
			float4 r = input[bx*blockDim.x + threadIdx.x];
			result[bx * blockDim.x + threadIdx.x] = sortElem(r); 
		}
}

__global__ void
mergeSortPass(const float4 *input, float4 *result, int nrElems, int threadsPerDiv) 
{
