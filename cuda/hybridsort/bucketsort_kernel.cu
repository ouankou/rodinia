...
__global__ void
bucketcount(const float *input, const float *pivot, int *indice, unsigned int *d_prefixoffsets, int size)
{
	...
	for (int tid = blockIdx.x * blockDim.x + threadIdx.x; tid < size; tid += numThreads) {
		...
		float piv = pivot[idx]; //s_pivotpoints[idx]; 

		while(jump >= 1){
			idx = (elem < piv) ? (idx - jump) : (idx + jump);
			piv = pivot[idx]; //s_pivotpoints[idx]; 
			jump /= 2; 
		}
